...
class Bullets(pygame.sprite.Sprite):
  ...
  def shoot(self):
    self.cooldown()
    if self.cool_down_counter == 0 and self.bullets:
        bullet = Bullets(RED, 2 , player.player_x() , player.rect.y, invader_group)
        bullet_group.add(bullet)
        self.cool_down_counter = 1
        self.bullet_count -= 1
        print("bulletcount = ",self.bullet_count)
...
# for _ in range (50):
#   bullet = Bullets(RED, 0 , 0 )
#   bullet_group.add (bullet)
#we cannot add all of the bullets into the bullet_group before the space key is pressed 
# group is mixed up therefore we do not know which element to shoot first. 
# Create a list of the invader blocks
invader_group = pygame.sprite.Group() 
# Create a list of all sprites hello
all_sprites_group = pygame.sprite.Group()
player = Player(YELLOW,0 )
# Create the invaderflakes
number_of_flakes = 10 # we are creating in invaders
all_sprites_group.add (player)
for x in range (number_of_flakes):
  my_invader = Invader(10, 10 , random.randint(0,49) , 1) # invaderflakes are white with size 5 by 5 px
...
bullet = Bullets(RED, 0 , player.player_x(), player.rect.y, invader_group)
#Next x
